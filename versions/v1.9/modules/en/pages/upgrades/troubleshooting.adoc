...
[IMPORTANT]
====
Operating a SUSE Virtualization cluster with mixed node versions is a transient state intended to last for hours or days, not months. Leaving a cluster in a split-version state for months introduces several severe risks. While an upgrade is paused or active, infrastructure changes should be frozen, and we recommend the primary focus should be troubleshooting and resolving the issues preventing the VM migration.
====
