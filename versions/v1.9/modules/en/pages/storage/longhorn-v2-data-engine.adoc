...
=== Stalled I/O operations on ARM platforms

On ARM64 platforms, Longhorn V2 disks that use the SPDK NVMe bdev driver can experience stalled I/O operations. For more information, see issue https://github.com/harvester/harvester/issues/10710[#10710].

To work around the issue, perform the following steps:

. xref:hosts/hosts.adoc#_remove_disks[Remove the affected Longhorn V2 disk].

. xref:hosts/hosts.adoc#_add_disks[Add the Longhorn V2 disk again].

. Verify that the workaround was successful.
+
[,shell]
----
...
[NOTE]
====
Starting with {harvester-product-name} v1.9.0, the default disk driver for Longhorn V2 volumes is changed to `AIO`. Removing and then re-adding the affected disk forces {harvester-product-name} to use this new default driver and resolves the stalled I/O operations on ARM64 platforms.
====
