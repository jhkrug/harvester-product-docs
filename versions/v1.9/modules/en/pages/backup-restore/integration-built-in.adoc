= Built-in Virtual Machine Backup, Snapshot & Restore
:revdate: 2026-01-08
:page-revdate: {revdate}

{harvester-product-name-tm} includes native backup, snapshot, and restore functionality for virtual machines that use {longhorn-product-name-tm} volumes, requiring no additional software. This page describes how to configure and use it. For an overview of how it compares to the other supported tools, see xref:backup-restore/tools.adoc[Backup Tools]; for the terms used here, such as backup, snapshot, and the recovery objectives that guide scheduling, see xref:backup-restore/definitions.adoc[Backup Definitions].
...
[NOTE]
====
A backup target must be configured. For more information, see <<#_configuring_backup_target>>. If no backup target is set, a message prompts you to configure one.
...
+
* Earlier than v1.4.0: You must <<#_uploading_vm_images_new_cluster,upload>> and configure the virtual machine images on the new cluster. Ensure that the image names and configuration are identical so that the virtual machines can be restored.

[#_uploading_vm_images_new_cluster]
==== Uploading the same virtual machine images to a new cluster
...
This feature is particularly valuable for virtual machines with high I/O activity or critical data that requires point-in-time consistency guarantees. It provides the application-consistent backups discussed in xref:backup-restore/best-practices.adoc[Best Practices], and uses the same KubeVirt `virt-freezer` mechanism described for xref:backup-restore/integration-velero.adoc[Velero].
...
[#_scheduled_vm_backups_snapshots]
== Scheduled virtual machine backups and snapshots

{harvester-product-name} supports the creation of virtual machine backups and snapshots on a scheduled basis, with the option to retain a specific number of backups and snapshots. You can suspend, resume, and update the schedule at runtime. Set the schedule frequency to meet the xref:backup-restore/recommendations.adoc[Recovery Point Objective (RPO)] of the workload.
...
Stagger backup and snapshot schedules to minimize overlaps and avoid overloading the control plane and storage layer. Allow enough time between jobs for the previous operation to finish and for associated resources to be garbage-collected. If a scheduled backup starts while a previous backup is still running, {harvester-product-name} skips the new execution instead of queuing it and waits for the next scheduled run.
====
+
...
+
Backups and snapshots that are marked *Ready* can be used to restore the source virtual machine. For more information, see <<#_vm_backup_restore>> and <<#_vm_snapshot_restore>>.
+
...
For best results, use the <<#_scheduled_vm_backups_snapshots>> feature in {harvester-product-name}, which has safeguard mechanisms that mitigate heavy I/O when possible. Again, {harvester-product-name} does not support recurring {longhorn-product-name} snapshots and backups.
====

== Limitations and considerations

Before relying on the built-in functionality, be aware of the following limitations. Where these are significant for your environment, consider one of the tools described in xref:backup-restore/tools.adoc[Backup Tools].

{longhorn-product-name} volumes only::
Built-in backup and snapshot support is limited to {longhorn-product-name} volumes. Virtual machines that use volumes provisioned by an external CSI driver cannot be protected with the built-in feature; use a Kubernetes-native tool such as xref:backup-restore/integration-velero.adoc[Velero] instead.

Longhorn V2 Data Engine backups disabled::
In v1.9.0, virtual machine backups for Longhorn V2 Data Engine volumes are disabled while the backup workflow transitions to CDI. For more information, see https://github.com/harvester/harvester/issues/11151[harvester/harvester#11151].

Scheduling interval and overlap::
Scheduled backups and snapshots must have an interval of *at least one hour*. Stagger schedules to minimize overlaps and allow enough time for each operation and its resource garbage collection to finish. This bounds the shortest xref:backup-restore/definitions.adoc[RPO] achievable with the built-in scheduler. Workloads that require a shorter RPO need a different approach.

Consistency depends on guest configuration::
Application-consistent backups rely on filesystem freeze and thaw, which depend on guest virtual machine configuration that is _not controlled by {harvester-product-name}_. RHEL and SLE Micro may require a custom SELinux policy, and Windows requires the Volume Shadow Copy Service. See <<Filesystem freeze for virtual machine backups and snapshots>>.

Downstream guest clusters not protected::
Backing up and restoring the virtual machines that host a downstream guest cluster does not ensure that the guest cluster is restored correctly. Follow the backup and restore procedures for the Kubernetes distribution running in the virtual machines. For example, see https://docs.rke2.io/datastore/backup_restore[Backing up and restoring an RKE2 cluster].

Guest network preparation for restore::
Restored virtual machines retain the machine ID of the original. Virtual machines running Ubuntu managed by `netplan` must be configured with `dhcp-identifier: mac` before backup to avoid duplicate IP addresses and network conflicts on restore. See the note in <<Create a Virtual Machine Backup>>.

Interaction with cluster upgrades::
Virtual machine backups, snapshots, and active schedules block {harvester-product-name} upgrades. You must ensure no backups or snapshots are in use and suspend all schedules before upgrading. See <<Virtual Machine Operations and {harvester-product-name} Upgrades>>.

No integration with recurring {longhorn-product-name} jobs::
The built-in feature does not support or integrate {longhorn-product-name} recurring snapshot and backup jobs, because they conflict with certain operations and can generate heavy I/O without {harvester-product-name}'s awareness. Use the built-in <<#_scheduled_vm_backups_snapshots>> feature instead.

Single-cluster scope::
The built-in feature protects one cluster and offers no centralized, multi-cluster management. Restoring to another cluster is possible but requires manual setup, such as configuring the same backup target and matching virtual machine images. See <<Restore a new virtual machine on another {harvester-product-name} cluster>>.